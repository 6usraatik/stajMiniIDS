...
// port taramasý alarmý için eþik deðeri (kolay olsun diye 4)
...
void packet_handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data) {
    ...
    if (protokol == 0x0806) {
        ...
        if (ip_mac_tablosu.find(gonderen_ip) == ip_mac_tablosu.end()) {
            ...
            if (bilinen_mac != gonderen_mac) { // test için baþta == yapýldý sonradan != þekline çevrildi
                alarm_sayisi++;
                std::cout << "\nKRITIK ALARM: ARP SPOOFING! -> IP: " << gonderen_ip << "\n";

                // kara listeye alma
                kara_liste.insert(gonderen_ip);
                std::cout << "[+] " << gonderen_ip << " KARA LISTEYE EKLENDI!\n";

                if (log_dosyasi.is_open()) {
                    log_dosyasi << "[ALARM - ARP] Sahte MAC! IP: " << gonderen_ip << " Kara Listeye Alindi.\n";
                }
            }
        }
    }
        ...
        if (kara_liste.find(s_ip) != kara_liste.end()) {
            ...
            if (log_dosyasi.is_open()) {
                log_dosyasi << "[KARA LISTE TRAFIGI] " << s_ip << " sistemle iletisim kurmaya calisti.\n";
            }
            return; // kara listedeki adresi atla
        }
        ...
        if (h_port != 0) {
            ...
            // eþik deðer aþýldýysa alarm ver
            if (port_tarama_takibi[s_ip].size() == PORT_TARAMA_LIMITI) {
                alarm_sayisi++;
                port_tarama_sayisi++;
                std::cout << "\n=============================================================\n";
                std::cout << " KRITIK GUVENLIK ALARMI: PORT TARAMASI (NMAP) TESPITI!\n";
                std::cout << "=============================================================\n";
                std::cout << " -> Saldirgan IP : " << s_ip << "\n";
                std::cout << " -> Durum        : Kisa surede " << PORT_TARAMA_LIMITI << " farkli portu yokladi!\n";
                std::cout << " -> Aksiyon      : IP Adresi KARA LÝSTEYE alindi.\n";
                std::cout << "=============================================================\n";
                ...
                if (log_dosyasi.is_open()) {
                    log_dosyasi << "[ALARM - PORT SCAN] " << s_ip << " port taramasi yapti! Kara listeye eklendi.\n";
                }
            }
        }
    }
...
int main() {
    ...
    std::cout << "\nAg dinleniyor... (Port Taramasý Korumasý ve Kara Liste Aktif)\n";
